...
function ChatContainer({setShowProfile}) {
  ...
  return selectedUser ? (
    <div className='h-full min-h-0 overflow-hidden relative backdrop-blur-lg'>
        {/*------- header -------*/}
        <div className='flex items-center gap-3 py-3 mx-3 sm:mx-4 border-b border-stone-500'>
          <img onClick={()=>setShowProfile(true)} src={selectedUser.profilePic || assets.avatar_icon} alt="" className='w-8 h-8 rounded-full cursor-pointer md:cursor-default'/>
          <p onClick={()=>setShowProfile(true)} className='flex-1 min-w-0 text-base sm:text-lg text-white flex items-center gap-2 cursor-pointer md:cursor-default'>
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-2 sm:gap-3 p-3'>
          <div className='flex-1 min-w-0 flex items-center bg-gray-100/12 px-3 rounded-full'>
              <input onChange={(e)=>setInput(e.target.value)} 
              value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMessage(e) : null} type="text" placeholder="Send a message" className='flex-1 min-w-0 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400'/>
